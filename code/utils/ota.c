...
static void ota_list_progress(int cur_recv_len, int total_size, int *cur_progress)
{
    int progress;
    int report_inter_len, next_report_len;

    report_inter_len = total_size / OTA_PROGRESS_REPORT_INTERV;
    progress = *cur_progress;
    next_report_len = report_inter_len * (progress + 1);
    
    if (!report_inter_len || cur_recv_len < next_report_len) {
        return;
    }

    progress = cur_recv_len / report_inter_len;
    at_rsp("Report interval[%d], Progress: %d...\n", report_inter_len, progress * OTA_PROGRESS_REPORT_STEP);
#if ZG_BUILD
    s907x_zg_ota_report_progress(progress * OTA_PROGRESS_REPORT_STEP);
#endif
...
int ota_http_download_new_fw(char *flash_buf, char *rsp_buf, void *httpc_cli, void *ota_header)
{
    ...
    do {
        ...
        ota_list_progress(total_recv_len, rsp_content_len, &cur_progress);
#if CONFIG_OTA_DUAL_IMAGES
...
int ota_http_update_image(void)
{
    ...
    httpc_ret = ota_http_request_new_fw(ota_bin_path, httpc_cli);
    if (HTTPC_OK != httpc_ret) {
...
{
    return g_st_ota_flash.addr;
}

void ota_set_bin_url(const char *bin_url, const int bin_url_len)
{
    int len = bin_url_len;

    if (OTA_TEST_URL_LEN_MAX <= len) {
        AT_DBG_ERR("ota URL len[%d] exceed max[%d].", len, OTA_TEST_URL_LEN_MAX);
        len = OTA_TEST_URL_LEN_MAX - 1;
    }
    
    memset(g_ac_ota_bin_url, 0, OTA_TEST_URL_LEN_MAX);
    memcpy(g_ac_ota_bin_url, bin_url, len);
}
char * ota_get_bin_url(void)
{
    return g_ac_ota_bin_url;
}

void ota_set_bin_name(const char *bin_name, const int bin_name_len)
{
    int len = bin_name_len;

    if (OTA_TEST_BIN_NAME_LEN <= len) {
        AT_DBG_ERR("ota name len[%d] exceed max[%d].", len, OTA_TEST_BIN_NAME_LEN);
        len = OTA_TEST_BIN_NAME_LEN - 1;
    }
    memset(g_ac_ota_bin_name, 0, OTA_TEST_BIN_NAME_LEN);
    memcpy(g_ac_ota_bin_name, bin_name, len);
}
char * ota_get_bin_name(void)
{
    return g_ac_ota_bin_name;
}
